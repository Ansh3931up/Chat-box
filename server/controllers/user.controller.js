...
const getAllChatUsers= asyncHandler(async (req, res) => {
    const user = await User.findById(req.user._id).populate('allchats');
    if (!user) {
        return res.status(404).json(new ApiError(404, "User not found"));
    }
    return res.status(200).json(new ApiResponse(200, user.allchats, "Fetched chat users successfully"));
});

const postallChatUsers = asyncHandler(async (req, res) => {
    ...
    if (keys.length > 0) {
        ...
        return res.status(400).json(new ApiError(400, "No userId found in request body"));
    }
    ...
    // Ensure user2's ID is not already in user1's allchats
    if (!user1.allchats.includes(user2._id)) {
        user1.allchats.push(user2._id); // Push user2's ID
    }

    // Ensure user1's ID is not already in user2's allchats
    if (!user2.allchats.includes(user1._id)) {
        user2.allchats.push(user1._id); // Push user1's ID
    }
    ...
    return res
        .status(200)
        .json(new ApiResponse(200,  "Chat users added successfully"));
});




const filterbyfullname=asyncHandler(async(req,res)=>{
    console.log("body",req.body);
...
export {register,filterbyfullname,getAllUsers,postallChatUsers,login,logout,getAllChatUsers,getProfile,forgot,reset,updateAccountDetails,changeCurrentPassword,getCurrentUser};
